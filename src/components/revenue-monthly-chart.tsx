...
import type { MonthlyRevenuePoint } from "@/lib/revenue-dashboard";
...
function ChartTooltip({
  ...
  active?: boolean;
  payload?: { payload: MonthlyRevenuePoint }[];
}) {
...
export function RevenueMonthlyChart({ data }: { data: MonthlyRevenuePoint[] }) {
    ...
    return (
      <Card>
        ...
        <p className="mt-2 text-sm text-slate-600">
          No revenue data since January 2026. Try clearing filters or refresh the Ops File
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            From January 2026 to the last full month.
          </p>
        </div>
      ...
      <div className="h-80 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey="revenue" fill="#7c3aed" radius={[4, 4, 0, 0]} maxBarSize={64} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </Card>
